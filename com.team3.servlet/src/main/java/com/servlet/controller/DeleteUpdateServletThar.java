...
public class DeleteUpdateServletThar extends HttpServlet{

	private static final long serialVersionUID = 1L;
	
	public DeleteUpdateServletThar() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result;
		try {
			result = imp.deleteThar(mahindra);
			RequestDispatcher success = request.getRequestDispatcher("/DeleteThar.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result;
		try {
			result = imp.updateThar(entity);
		
		
		HttpSession session = request.getSession();
		session.setAttribute("updateThar", entity);
