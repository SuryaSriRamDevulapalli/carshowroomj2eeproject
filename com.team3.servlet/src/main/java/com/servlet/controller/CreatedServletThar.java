...
public class CreatedServletThar extends HttpServlet {
    private static final long serialVersionUID = 1L;

    public CreatedServletThar() {
        super();
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException{
        ...
        Implementation imp = new Implementation();
			try {
				int result = imp.InsertmethodThar(mahindra);
			
	
			HttpSession session = request.getSession();
			session.setAttribute("TharInsert", mahindra);
        
        
			RequestDispatcher success = request.getRequestDispatcher("/SuccessThar.jsp");
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) {
    	...
    	Implementation imp = new Implementation();
    	try {
			Servletentity entity = imp.SearchThar(variant);
			HttpSession session = request.getSession();
			session.setAttribute("mahindraobject", entity);
			
			RequestDispatcher success = request.getRequestDispatcher("/MahhindraTharServlet");
			RequestDispatcher failed = request.getRequestDispatcher("/Failure.html");
